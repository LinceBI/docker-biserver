...
find "${BISERVER_INITD:?}" /home/biserver/ -type d -not -perm 0775 -exec chmod -c 0775 '{}' '+'
find "${BISERVER_INITD:?}" /home/biserver/ -type f -not '(' -perm 0664 -o -regex '.*\.\(sh\|run\)$' ')' -exec chmod -c 0664 '{}' '+'
find "${BISERVER_INITD:?}" /home/biserver/ -type f '(' -not -perm 0775 -a -regex '.*\.\(sh\|run\)$' ')' -exec chmod -c 0775 '{}' '+'
