...
find "${BISERVER_PRIV_INITD:?}" "${BISERVER_INITD:?}" -type f -not '(' -perm 0664 -o -regex '^.*\.\(sh\|run\)$' ')' -exec chmod -c 0664 '{}' '+'
find "${BISERVER_PRIV_INITD:?}" "${BISERVER_INITD:?}" -type f '(' -not -perm 0775 -a -regex '^.*\.\(sh\|run\)$' ')' -exec chmod -c 0775 '{}' '+'
