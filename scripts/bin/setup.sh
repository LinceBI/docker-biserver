...
# shellcheck disable=SC2034
{
	...
	LDAP_ADMIN_USER=$(getVar LDAP_ADMIN_USER "cn=admin,ou=users,dc=example,dc=localdomain")

	COOKIE_SESSION_TIMEOUT=$(getVar COOKIE_SESSION_TIMEOUT "120")

	CAS_ENABLED=$(getVar CAS_ENABLED "false")
	CAS_PUBLIC_URL=$(getVar CAS_PUBLIC_URL "${FQSU_PROTOCOL:?}://${FQSU_DOMAIN:?}:${FQSU_PORT:?}/auth/realms/biserver/protocol/cas")
	CAS_PRIVATE_URL=$(getVar CAS_PRIVATE_URL "${CAS_PUBLIC_URL:?}")
	...
	CAS_TICKETVALIDATOR_URL=$(getVar CAS_TICKETVALIDATOR_URL "${CAS_PRIVATE_URL:?}")
	CAS_LOGIN_URL=$(getVar CAS_LOGIN_URL "${CAS_PUBLIC_URL:?}/login")
	CAS_LOGOUT_URL=$(getVar CAS_LOGOUT_URL "${CAS_PUBLIC_URL:?}/logout")
