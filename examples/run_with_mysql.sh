...
MYSQL_CONTAINER_MAINTENANCE_PASSWORD='H4!b5at+kWls-8yh4Guq'
...
"${DOCKER:?}" run --detach \
	...
	--env MYSQL_ROOT_PASSWORD="${MYSQL_CONTAINER_MAINTENANCE_PASSWORD:?}" \
	"${MYSQL_IMAGE_NAME:?}"
...
"${DOCKER:?}" run --detach \
	...
	--env MYSQL_MAINTENANCE_PASSWORD="${MYSQL_CONTAINER_MAINTENANCE_PASSWORD:?}" \
	"${BISERVER_IMAGE_NAME:?}" "$@"
